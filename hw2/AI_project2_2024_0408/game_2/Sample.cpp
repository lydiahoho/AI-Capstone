...
#define ITERS 500
#define TO 2900
int total_n = 0, init_x = 0, init_y = 0, component = 0;
...
std::vector<int> InitPos(int mapStat[15][15])
{
	...
	vector<int> step_size[15][15]; // size = 8, for 8 directions
			...
			if (mapStat[x][y] == 0 && ava[x][y] == 1)
			{
				...
				for (int dir = 1; dir <= 9; dir++)
				{
					...
					while (true)
					{
						...
						{
							if (x_step < 14 && y_step > 0 && mapStat[x_step + 1][y_step - 1] == 0)
							{
								x_step++;
								y_step--;
								step_length++;
						...
						{
							if (x_step < 14 && mapStat[x_step + 1][y_step] == 0)
							{
								x_step++;
								step_length++;
							}
						...
						{
							if (x_step > 0 && y_step < 14 && mapStat[x_step - 1][y_step + 1] == 0)
							{
								x_step--;
								y_step++;
								step_length++;
						...
						{
							if (y_step < 14 && mapStat[x_step][y_step + 1] == 0)
							{
								y_step++;
								step_length++;
							}
						...
						{
							if (x_step < 14 && y_step < 14 && mapStat[x_step + 1][y_step + 1] == 0)
							{
								x_step++;
								y_step++;
								step_length++;
...
int expansion(int sel_node, vector<struct node>& tree, int playerID)
{
		...
		for (int x = 0; x < 15; x++)
		{
			...
			{	// probably canMove
				 ...
				 for (int dir = 0; dir < 9; dir++)
				{
					...
					if (temp_x >= 0 && temp_x < 15 && temp_y >= 0 && temp_y < 15 && tree[sel_node].board[temp_x][temp_y] == 0)
					{
						ava_dir++;
				...
				for (int dir : possible_dirs)
				{
					...
					int temp_y = y;
					int step_length = 0;
					while (temp_x + delta_x[dir - 1] >= 0 && temp_x + delta_x[dir - 1] < 15 &&
						temp_y + delta_y[dir - 1] >= 0 && temp_y + delta_y[dir - 1] < 15 &&
						tree[sel_node].board[temp_x + delta_x[dir - 1]][temp_y + delta_y[dir - 1]] == 0)
					{
						temp_x += delta_x[dir - 1];
						temp_y += delta_y[dir - 1];
						step_length++;						
					}
					vector<int> temp_step = { x, y, 0, dir, temp_x, temp_x};
					if (ava_dir == 1)
					{
						...
						if (step_length < 4) {
							for (int j = 1; j <= 9; j++) {
								if (j == 5) continue;
								if ((tree[sel_node].board[temp_x + delta_x[j - 1]][temp_y + delta_y[j - 1]] == 0)) {
									empty++;
								}
							}
	...
	if (canMove)
	{
		...
		for (int k = 0; k < tree[sel_node].step.size(); k++)
		{
			...
			int x_step = tree[sel_node].step[k][4];
			int y_step = tree[sel_node].step[k][5];
...
void dfs(int(&gameBoard)[15][15], int x, int y, int playerID)
{
	gameBoard[x][y] = 0;
	if (y < 14 && gameBoard[x][y + 1] == playerID)
	{
		component++;
		dfs(gameBoard, x, y + 1, playerID);
	}
	if (x < 14 && gameBoard[x + 1][y] == playerID)
	{
		component++;
...
{ // player's order
	...
	while (!terminate)
	{
		...
		for (int y = 0; y < 15; y++)
		{
			for (int x = 0; x < 15; x++)
			{
				...
				{ // probably canMove
					...
					if (x < 14 && y > 0 && gameBoard[x + 1][y - 1] == 0)
					{
					...
					if (x > 0 && y < 14 && gameBoard[x - 1][y + 1] == 0)
					{
					...
					if (x < 14 && y < 14 && gameBoard[x + 1][y + 1] == 0)
					{
		...
		else
		{
			...
			{ // terminate condition: border, occupied
				...
				{
					if (x_step < 14 && y_step > 0 && gameBoard[x_step + 1][y_step - 1] == 0)
					{
						x_step++;
						y_step--;
					}
				...
				{
					if (x_step < 14 && gameBoard[x_step + 1][y_step] == 0)
					{
						x_step++;
					}
					else
				...
				{
					if (x_step > 0 && y_step < 14 && gameBoard[x_step - 1][y_step + 1] == 0)
					{
						x_step--;
						y_step++;
					}
				...
				{
					if (y_step < 14 && gameBoard[x_step][y_step + 1] == 0)
					{
						y_step++;
					}
					else
				...
				{
					if (x_step < 14 && y_step < 14 && gameBoard[x_step + 1][y_step + 1] == 0)
					{
						x_step++;
						y_step++;
					}
	...
	for (double s : total_score)
	{
		if(s > player_score) rank++;
	}
	return rank;
}
